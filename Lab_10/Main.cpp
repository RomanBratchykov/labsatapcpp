...
int main() {
    ...
    while (1){
        ...
        for (int i = 0; i < size; i++) {
            ...
            switch (bookType) {
                ...
                case 2:
                {
                    ...
                    books[i] = Book(name, author, yearOfPublish, numberOfPages, publisher, city);
                    break;
                }
                case 3: {
        ...
        std::cout << "Enter number to choose publisher :\n";
        for (int i = 0; i < 5; i++){
            std::cout << i + 1 << ". "<< publishers[i] << "\n";
        }
        int index;
        std::cin >> index;
        std::cout << "Number of books with publisher " << publishers[index - 1] << ": " << numberOfPublishers(books, size, publishers[index - 1]) << "\n";
        std::cout << "Books from library:\n";
        Library library("Fiction");
        for (int i = 0; i < size; i++) {
            library.show(books[i]);
        }
